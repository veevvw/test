...
def remove_plugin(shortname):
    ...
    try:
        name = f"Matrix.plugins.{shortname}"
        for i in reversed(range(len(sedub._event_builders))):
            ev, cb = sedub._event_builders[i]
            if cb.__module__ == name:
                del sedub._event_builders[i]
