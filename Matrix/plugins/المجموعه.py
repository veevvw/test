...
@sedub.zed_cmd(pattern="تجميع الاعضاء$")
...
@sedub.zed_cmd(pattern="اضف الاعضاء$")
...
@sedub.zed_cmd(pattern="رفع الحظر ?(.*)")
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
...
@sedub.zed_cmd(pattern=r"مسح المحظورين(.*)")
async def _(event):
...
@sedub.zed_cmd(pattern=r"المحذوفين ?([\\s\\S]*)")
...
@sedub.zed_cmd(pattern="احصائياتي$")
...
@sedub.zed_cmd(pattern="(الرسائل|رسائل) ?(.*)")
async def zed(event):
    k = await event.get_reply_message()
    if k:
        a = await bot.get_messages(event.chat_id, 0, from_user=k.sender_id)
...
@sedub.zed_cmd(pattern="الحاظرهم$")
async def main(event):
    result = await sedub(functions.contacts.GetBlockedRequest(offset=0, limit=1000000))
    alist = []
    for user in result.users:
        if not user.bot:
            ...
            alist.append(zzz)
    if alist:
        await sedub.send_message("me", "\n".join(alist))


@sedub.zed_cmd(pattern="قيد (.*)")
async def _(event):
    exe = event.text[5:]
    try:
        result = await sedub(
...
@sedub.zed_cmd(pattern="احذف (.*)")
async def _(event):
    exe = event.text[5:]
    await sedub.get_dialogs()
    chat = exe
    await sedub.delete_dialog(chat, revoke=True)
