...
@sedub.zed_cmd(pattern="(ضع اشتراك الخاص|وضع اشتراك الخاص)(?: |$)(.*)")
async def _(event):
...
@sedub.zed_cmd(pattern="(تفعيل اشتراك الخاص|تفعيل الاشتراك خاص)")
...
@sedub.zed_cmd(pattern="(تعطيل اشتراك الخاص|تعطيل الاشتراك الخاص)")
    ...
    if Matrixaal:
        try:
            ...
            if rip is False and not pmpermit_sql.is_approved(event.sender_id):
                c = await sedub.get_entity(ch)
                chn = c.username
                if c.username == None:
                    ra = await sedub(ExportChatInviteRequest(ch))
                    chn = ra.link
...
@sedub.zed_cmd(pattern="(ضع اشتراك الكروب|وضع اشتراك الكروب) ?(.*)")
async def fs(event):
...
@sedub.zed_cmd(incoming=True, func=lambda e: e.is_group, edited=False, forword=None)
async def fg(event):
...
@sedub.zed_cmd(pattern="تعطيل اشتراك الكروب$")
