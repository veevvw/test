...
from . import sedub, BOTLOG_CHATID
...
@sedub.zed_cmd(incoming=True, groups_only=True)
...
@sedub.zed_cmd(pattern="تحذير(?:\\s|$)([\\s\\S]*)")
async def _(event):
...
@sedub.zed_cmd(pattern="التحذيرات")
async def _(event):
    reply_message = await event.get_reply_message()
    if not reply_message:
...
@sedub.zed_cmd(pattern="حذف التحذيرات(?: |$)(.*)")
async def _(event):
    reply_message = await event.get_reply_message()
    sql.reset_warns(reply_message.sender_id, event.chat_id)
