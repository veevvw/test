...
async def super_function(event, Matrixal, zed, sleeptimet, er, done):
    # sourcery no-metrics
    counter = int(zed[0])
    num = 0
                    ...
                    try:
                        if Matrixal.text: #Write Code By T.me/zzzzl1l
                            ...
                            try: #Write Code By T.me/zzzzl1l
                                ...
                                await _zedutils.unsavegif(event, Matrixal)
                                await asyncio.sleep(sleeptimet)
                            except BaseException:
                                er += 1
                                await asyncio.sleep(sleeptimet)
...
@sedub.zed_cmd(pattern="ايقاف السوبرات$")
...
@sedub.zed_cmd(pattern="ايقاف سوبر$")
async def stopspamrz(event):
...
@sedub.zed_cmd(pattern="السوبرات$")
async def add_blacklist_chat(event):
    blkchats = blacklist_chats_list()
    try:
        blacklistchats = sql.get_collection("blacklist_chats_list").json
...
@sedub.zed_cmd(pattern="(ايقاف النشر|ستوب)(?: |$)(.*)")
async def _(event):
    if event.is_private:
        return await edit_or_reply(event, "**⎉╎عـذراً .. النشر التلقائي خـاص بالقنـوات/المجموعات فقـط\n⎉╎قم باستخـدام الامـر داخـل القنـاة/المجموعة الهـدف**")
    if input_str := event.pattern_match.group(2):
...
async def _(event): # Code by t.me/zzzzl1l
    ...
    result = await sedub(functions.contacts.SearchRequest(
        q=search,
        limit=20
    ))
    json = result.to_dict()
...
@sedub.zed_cmd(pattern="كلمه (.*)")
async def _(event): # Code by t.me/zzzzl1l
    search_word = event.pattern_match.group(1)
    chat = await event.get_chat()
    chat_name = chat.title
    l = 'qwertyuiopasdfghjklxcvbnmz'
    messages = await sedub.get_messages(chat, filter=InputMessagesFilterEmpty(), limit=100)
    i = str(''.join(random.choice(l) for i in range(3))) + '.txt'
    counter = 0
...
@sedub.zed_cmd(pattern="كريس")
async def cr7(crr): # Code by t.me/zzzzl1l
    await crr.edit(Z)
    


@sedub.zed_cmd(pattern="ماريو")
...
@sedub.zed_cmd(pattern="(كلبي|فكيو|ورده|سوفيت|كلوك|تحبني)$")
async def _(kst):
...
@sedub.zed_cmd(pattern="(شبح|دعبل)$")
async def _(kst):
    cmd = kst.pattern_match.group(1)
    if cmd == "شبح":
        expr = """
