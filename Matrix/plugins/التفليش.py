...
async def ban_user(chat_id, i, rights):
    try:
        await sedub(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)

@sedub.zed_cmd(pattern=r"اطردني(.*)")
async def kickme(leave):
    await leave.edit("**⎉╎جـاري مـغادرة المجـموعة مـع السـلامة  🚶‍♂️  ..**")
    await leave.client.kick_participant(leave.chat_id, "me")

@sedub.zed_cmd(pattern=r"مغادره(.*)")
async def banme(leave):
    await leave.edit("**⎉╎جـاري مـغادرة المجـموعة مـع السـلامة  🚶‍♂️  ..**")
    await leave.client.kick_participant(leave.chat_id, "me")

@sedub.zed_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**⎉╎البـوت المسـاعد الخـاص بك هـو** \n {TG_BOT_USERNAME}")

@sedub.zed_cmd(pattern="حالتي ?(.*)")
async def zze(event):
...
@sedub.on(events.NewMessage(pattern="/matrix"))
async def _(event):
...
@sedub.zed_cmd(pattern="(مغادرة الكروبات|مغادرة المجموعات)")
async def leave_groups(event):
    ...
    try:
        ...
        for group in gg:
            if group not in ss:
                await sedub.delete_dialog(group)
                num += 1
                await sleep(1)
        if num >=1:
...
@sedub.zed_cmd(pattern="مغادرة القنوات")
...
@sedub.zed_cmd(pattern="حذف الخاص")
async def _(event):
...
@sedub.zed_cmd(pattern="حذف البوتات")
async def _(event):
...
@sedub.zed_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
        await sedub(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in sedub.iter_participants(chat_id):
        ...
        await sedub.send_message(chat_id, usrtxt)
...
@sedub.zed_cmd(pattern=f"{HDRALL}$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
...
@sedub.zed_cmd(pattern=f"{KTMALL}$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
...
@sedub.zed_cmd(pattern="الغاء التفليش", groups_only=True)
...
@sedub.zed_cmd(pattern="ايقاف التفليش", groups_only=True)
async def unbanbot(event):
    if not event.chat_id in spam_chats:
        return await edit_or_reply(event, "**- لاتوجـد عمليـة تفليـش هنـا لـ إيقافـها ؟!**")
    else:
