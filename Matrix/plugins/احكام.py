...
from . import BOTLOG, BOTLOG_CHATID, mention, sedub
LOGS = logging.getLogger(__name__)


@sedub.zed_cmd(pattern="احكام(?: |$)(.*)")
...
# Code by t.me/zzzzl1l
    ...
    if gvarstatus("Z_AK") is not None and gvarstatus("Z_A2K") is not None and gvarstatus("Z_A3K") is None and gvarstatus("Z_A4K") is None and gvarstatus("Z_A5K") is None:
        ...
        zilzal = int(zee2)
        try:
            user_zed = await event.client.get_entity(Matrixal)
...
@sedub.zed_cmd(pattern="(ايقاف عقاب|انهاء عقاب)$")
async def zed(event): # Code by t.me/zzzzl1l
    if gvarstatus("Z_EKB") is None:
        delgvar("Z_EKM")
        delgvar("Z_EK")
    ...
    if gvarstatus("Z_EKB") is not None and event.chat_id == int(gvarstatus("E_CHAT")):
        ...
        if gvarstatus("Z_E2K") is None:
            ...
            try:
                ...
                u1 = await sedub(GetUsersRequest(zzz1))
            ...
            try:
                ...
                u2 = await sedub(GetUsersRequest(zzz2))
            zillzall = f"[{user.first_name}](tg://user?id={user.id})"
            zilzal1 = f"[{u1.first_name}](tg://user?id={u1.id})"
            ...
            try:
                ...
                u3 = await sedub(GetUsersRequest(zzz3))
            zillzall = f"[{user.first_name}](tg://user?id={user.id})"
            zilzal1 = f"[{u1.first_name}](tg://user?id={u1.id})"
            zilzal2 = f"[{u2.first_name}](tg://user?id={u2.id})"
            ...
            try:
                ...
                u4 = await sedub(GetUsersRequest(zzz4))
            zillzall = f"[{user.first_name}](tg://user?id={user.id})"
            zilzal1 = f"[{u1.first_name}](tg://user?id={u1.id})"
            zilzal2 = f"[{u2.first_name}](tg://user?id={u2.id})"
            zilzal3 = f"[{u3.first_name}](tg://user?id={u3.id})"
            ...
            try:
                ...
                u5 = await sedub(GetUsersRequest(zzz5))
            zillzall = f"[{user.first_name}](tg://user?id={user.id})"
            zilzal1 = f"[{u1.first_name}](tg://user?id={u1.id})"
            zilzal2 = f"[{u2.first_name}](tg://user?id={u2.id})"
            zilzal3 = f"[{u3.first_name}](tg://user?id={u3.id})"
...
@sedub.zed_cmd(pattern="ذاتي (\\d*) ([\\s\\S]*)")
...
@sedub.zed_cmd(pattern="ذااتي (\\d*) ([\\s\\S]*)")
async def selfdestruct(destroy):
    zzz = ("".join(destroy.text.split(maxsplit=1)[1:])).split(" ", 1)
    message = zzz[1]
    ttl = int(zzz[0])
...
@sedub.on(admin_cmd(pattern="(خط الغامق|خط غامق|تفعيل غامق|تفعيل الغامق)"))
async def _(event):
...
@sedub.on(admin_cmd(pattern="(تعطيل غامق|تعطيل الغامق)"))
async def _(event):
    is_thin = gvarstatus("thin")
    if is_thin:
        delgvar("thin")
...
@sedub.on(admin_cmd(pattern="(خط المائل|خط مائل|تفعيل مائل|تفعيل المائل)"))
async def _(event):
...
@sedub.on(admin_cmd(pattern="(تعطيل مائل|تعطيل المائل)"))
async def _(event):
    is_mael = gvarstatus("mael")
    if is_mael:
        delgvar("mael")
...
@sedub.on(admin_cmd(pattern="(خط التشويش|خط تشويش|تفعيل تشويش|تفعيل التشويش)"))
async def _(event):
...
@sedub.on(admin_cmd(pattern="(تعطيل تشويش|تعطيل التشويش)"))
async def _(event):
    is_cllear = gvarstatus("cllear")
    if is_cllear:
        delgvar("cllear")
...
@sedub.on(admin_cmd(pattern="(خط النسخ|خط نسخ|تفعيل نسخ|تفعيل النسخ)"))
async def _(event):
...
@sedub.on(admin_cmd(pattern="(تعطيل نسخ|تعطيل النسخ)"))
async def _(event):
    is_cood = gvarstatus("cood")
    if is_cood:
        delgvar("cood")
...
@sedub.on(events.NewMessage(outgoing=True))
