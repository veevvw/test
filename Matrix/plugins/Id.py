...
@sedub.zed_cmd(pattern="الصور(?:\\s|$)([\\s\\S]*)")
async def potocmd(event):
    uid = "".join(event.raw_text.split(maxsplit=1)[1:])
...
@sedub.zed_cmd(pattern="اضف كليشة (الايدي|ايدي|الفحص|فحص|الحماية|الحمايه|الخاص) ?(.*)")
...
@sedub.zed_cmd(pattern="اضف كليشه (الايدي|ايدي|الفحص|فحص|الحماية|الحمايه|الخاص) ?(.*)")
async def variable(event):
    input_str = event.pattern_match.group(1)
    reply = await event.get_reply_message()
    vinfo = reply.text
