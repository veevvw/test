...
async def get_names(phone_number):
    try:
        ...
        contacts = await sedub(functions.contacts.ImportContactsRequest([contact]))
...
@sedub.zed_cmd(pattern="اضف وسائط (الحماية|الحمايه|الفحص|فحص) ?(.*)")
async def _(malatha):
    if malatha.fwd_from:
        return
...
async def save_post(event):
    ...
    if post_link.startswith("https://t.me/c/"):
        ...
        try:
            ...
            return await edit_or_reply(event, f"**- اووبـس .. حدث خطأ أثناء حفظ الرسالة\n- تفاصيل الخطأ :**\n {str(e)}\n\n**- استخـدم الامـر الآخـر لـ حفـظ الملفـات المقيـده 🔳:\n- ارسـل** ( .احفظ ) **+ رابـط او بالـرد ع رابـط مقيـد**")
    try:
        message = await sedub.get_messages(channel_username_or_id, ids=message_id)
        if not message:
            return await edit_or_reply(event, "**- رابـط غيـر صالـح ؟!**")
        if message.media:
            file_ext = ""
            ...
            await sedub.download_media(message, file=file_path)
            if message.text:
                ahmed = await sedub.send_file(event.chat_id, file=file_path, caption=f"{message.text}")
                await sedub.send_message(event.chat_id, f"ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ - حـفـظ المـحتـوى 🧧\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⌔╎ تـم جلب المنشـور المقيـد .. بنجـاح ☑️** ❝\n**⌔╎رابـط المنشـور** {post_link} .", reply_to=ahmed)
                os.remove(file_path)
                await event.delete()
            else:
                await sedub.send_file(event.chat_id, file=file_path, caption=f"[ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ - حـفـظ المـحتـوى 🧧](t.me/VEEVVW) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⌔╎ تـم جلب المنشـور المقيـد .. بنجـاح ☑️** ❝\n**⌔╎رابـط المنشـور** {post_link} .")
                os.remove(file_path)
                await event.delete()
        else:
            if message.text:
                ali = await sedub.send_message(event.chat_id, f"{message.text}")
...
async def save_posts(event):
    ...
    os.makedirs(save_dir, exist_ok=True)
    try:
        channel_entity = await sedub.get_entity(channel_username)
        messages = await sedub.get_messages(channel_entity, limit=limit)
    except Exception as e:
    ...
    for message in messages:
        try:
            if message.media:
                ...
                await sedub.send_file("me", file=file_path)
                os.remove(file_path)
            if cancel_process:
                await event.edit("**- تم إلغـاء عمليـة حفـظ الميـديا .. بنجـاح✅**")
                cancel_process = False
...
@sedub.zed_cmd(pattern="متحركاات(?: |$)(.*)")
...
@sedub.zed_cmd(pattern="(لمتحركه|لمتحركة|متحركه|متحركة)$")
async def Matrixal_gif(event):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await sedub(unblock("VideoToGifConverterBot"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_file(reply_message)
            await conv.get_response()
...
@sedub.zed_cmd(pattern="(معالجه|تنقيه|تحسين|توضيح)$")
async def Matrixal_ai(event):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await sedub(unblock("PhotoFixerBot"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message("/start")
            await conv.get_response()
...
async def Matrixal_ss(event):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await sedub(unblock("download_story_tele_bot"))
            purgeflag = await conv.send_message(Matrixal)
        await conv.get_response()
...
@sedub.zed_cmd(pattern="(انمي|كارتون)$")
async def Matrixal_anime(event):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await sedub(unblock("qq_neural_anime_bot"))
            await conv.send_message("/start")
            await conv.get_response()
            purgeflag = await conv.send_file(reply_message)
        await conv.get_response()
...
async def Matrixal_scanner(event):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await sedub(unblock("rrobbootooBot"))
            await conv.send_message("/start")
            await conv.get_response()
            purgeflag = await conv.send_file(reply_message)
            #await conv.send_message("/ocr", reply_to=purgeflag)  # إرسال /ocr بالرد على الصورة داخل البوت
...
async def Matrixal_rr(event):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await sedub(unblock("bgkillerbot"))
            await conv.send_message("/start")
            await conv.get_response()
            purgeflag = await conv.send_file(reply_message)
        await conv.get_response()
...
@sedub.on(events.NewMessage(outgoing=True, pattern='.موقع (.*)'))
async def _(event):
    con = event.pattern_match.group(1) 
    sender = await event.get_sender()
    if sender.id != sedub.uid:
        return
    ...
    channel_entity = await sedub.get_entity(gpsbb)
    await sedub.send_message(gpsbb, '/start')
    await asyncio.sleep(0.5)
    msg0 = await sedub.get_messages(gpsbb, limit=1)
    await sedub.send_message(gpsbb, con)
    await asyncio.sleep(0.5)
    try:
        msg1 = await sedub.get_messages(gpsbb, limit=1)
    ...
    await asyncio.sleep(0.5)
    msg2 = await sedub.get_messages(gpsbb, limit=1)
    await msg2[0].click(2)
    await asyncio.sleep(0.5)
    msg3 = await sedub.get_messages(gpsbb, limit=1)
    await msg3[0].click(2)
    await asyncio.sleep(0.5)
    msg4 = await sedub.get_messages(gpsbb, limit=1)
    ...
    await asyncio.sleep(0.5)
    msg5 = await sedub.get_messages(gpsbb, limit=1)
    await sedub.send_file(
        event.chat_id,
        msg5[0],
...
@sedub.zed_cmd(pattern="(عدسه|عدسة)$")
async def _(event): # Write code by T.me/zzzzl1l
    ...
    channel_entity = await sedub.get_entity(zellzall)
    zzz = await event.edit("**⎉╎جـارِ البحث في عدسـة جـوجـل ...🔍**")
    channel_entity = await sedub.get_entity(zellzall)
    await sedub.send_message(zellzall, '/start')
    await asyncio.sleep(2)
    msga = await sedub.get_messages(zellzall, limit=1)
    await sedub.send_file(zellzall, reply_message)
    await asyncio.sleep(2)
    msg1 = await sedub.get_messages(zellzall, limit=1)
    msg2 = await sedub.get_messages(zellzall, limit=1)
    await asyncio.sleep(3)
    try: # Write code by T.me/zzzzl1l
        ...
        try:
            list = await sedub(GetHistoryRequest(peer=channel_entity, limit=2, offset_date=None, offset_id=0, max_id=0, min_id=0, add_offset=0, hash=0))
            ...
            list = await sedub(GetHistoryRequest(peer=channel_entity, limit=3, offset_date=None, offset_id=0, max_id=0, min_id=0, add_offset=0, hash=0))
...
async def Matrixal_insta(event):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await sedub(unblock("story_repost_bot"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message(link)
            zedthon = await conv.get_response()
...
async def Matrixal_insta(event):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await sedub(unblock("downloader_tiktok_bot"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message(link)
            zedthon = await conv.get_response()
...
@sedub.zed_cmd(pattern="(النص|لنص)$")
async def Matrixal_ai(event):
    reply_message = await event.get_reply_message()
    if not reply_message:
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await sedub(unblock("Speechpro_ASR_bot"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.get_response()
            await conv.send_message("/lang ara")
...
async def Matrixal_gif(event):
    ...
    async with borg.conversation(chat) as conv: 
        try:
            ...
            await sedub(unblock("GoogleBooksSearchBot"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message(Matrixal)
        try:
...
async def Matrixal_gpt(event):
    ...
    async with borg.conversation(chat) as conv: 
        try:
            ...
            await sedub(unblock("gpt3_unlim_chatbot"))
            purgeflag = await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message(question)
...
@sedub.zed_cmd(pattern="(pdf|نص pdf)$")
async def Matrixal_ai(event):
    reply_message = await event.get_reply_message()
    if not reply_message:
...
@sedub.on(events.NewMessage(outgoing=True, pattern='.صورة pdf$'))
async def _(event):
    reply_message = await event.get_reply_message()
    zzz = await edit_or_reply(event, "**- جـارِ طباعـة الصـورة الـى ملـف PDF . . .📕╰\n- الرجـاء الانتظـار ثوانـي . . .⏳╰**")
    channel_entity = await sedub.get_entity(ppdf)
    await sedub.send_message(ppdf, '/start')
    await asyncio.sleep(0.5)
    msg0 = await sedub.get_messages(ppdf, limit=1)
    await sedub.send_file(ppdf, reply_message)
    await asyncio.sleep(0.5)
    try:
        msg1 = await sedub.get_messages(ppdf, limit=1)
    ...
    await asyncio.sleep(0.5)
    msg2 = await sedub.get_messages(ppdf, limit=1)
    await msg2[0].click(0)
    await asyncio.sleep(0.5)
    msg3 = await sedub.get_messages(ppdf, limit=1)
    await asyncio.sleep(0.5)
    msg4 = await sedub.get_messages(ppdf, limit=1)
    await sedub.send_file(
        event.chat_id,
        msg4[0],
...
async def Matrixal_gif(event):
    ...
    async with borg.conversation(chat) as conv: 
        try:
            ...
            await sedub(unblock("SangMata_BOT"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message(f"{uid}")
        zlz = await conv.get_response()
...
async def Matrixal_ss(event):
    ...
    await sedub(JoinChannelRequest(channel="@logicxupdates"))
    async with borg.conversation(chat) as conv:
        try:
            purgeflag = await conv.send_message(link)
        except YouBlockedUserError:
            await sedub(unblock("Save_restricted_robot"))
            purgeflag = await conv.send_message(link)
        response = await conv.get_response()
        await asyncio.sleep(3)
        try:
...
@sedub.zed_cmd(pattern="(معرفاتي|يوزراتي)$")
async def _(event):
    ...
    await zzz.delete()
    await sedub.send_message(event.chat_id, output_str)
...
async def handle_event(event):
    ...
    await sedub.send_message(event.chat_id, split_message)
    await event.delete()

@sedub.zed_cmd(pattern=f"ت(?: |$)(.*)")
async def handle_event(event):
    malath = event.pattern_match.group(1)
    if malath:
        Matrixal = malath
