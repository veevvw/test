...
from . import edit_delete, sedub, logging
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await sedub.upload_file(autophoto_path)
        try:
            if i > 0:
                await sedub(
                    functions.photos.DeletePhotosRequest(
                        await sedub.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await sedub(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await sedub(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await sedub(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
sedub.loop.create_task(digitalpicloop())
sedub.loop.create_task(autoname_loop())
sedub.loop.create_task(autobio_loop())
